...
it('should login with valid credentials', () => {
    ...
    //
    cy.get('.sidebar-body > :nth-child(1) > :nth-child(22)').click()
    cy.get('.btn').click()
    cy.get('[name="partnership_name"]').click()
    cy.get('[name="partnership_name"]').type('hummasoft')
    cy.get('input[type="file"]').eq(0).selectFile('cypress/fixtures/hma.jpeg', { force: true })
    cy.get('#submitButton').click()
    cy.get('.swal2-confirm').click()
    cy.get('#searchName').click()
    cy.get('#searchName').type('hummasoft')
    ...
    cy.get('[name="partnership_name"]').click()
    cy.get('[name="partnership_name"]').clear()
    cy.get('[name="partnership_name"]').type('humm')
    cy.get('#submitButton').click()
    cy.get('.swal2-confirm').click()
